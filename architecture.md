...
`.astro` components make decomposition free at runtime: they compile to HTML, carry no lifecycle, and expose props as their only interface. With near-zero decomposition cost and positive containment value, extraction is the default, not the exception: within a page, any section that can be named is a component, and no reuse justification is required. Reuse across pages is a separate concern — the design-component context (`shared/components/`) — while per-page extraction is about discovering the boundaries a page already contains. Repeated script wiring is the strongest such signal: the same behavior appearing twice marks a component boundary. Two guards keep this from tipping into fragmentation: split wide rather than deep (one level of composition per page; no pass-through components that only forward props), and never extract what cannot be named.
